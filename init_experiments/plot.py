...
def plot_trajectories(summary_data, y_label, ylim, figsize=(6, 6), options=''):
    ...
    ax.set_ylabel(y_label + ' +/- std', fontsize=config.Figs.axis_fs)
    ...
    for x, y, std, label, n in summary_data:
        color = next(colors)
        ax.fill_between(x, np.clip(y + std, ylim[0], ylim[1]), y - std, alpha=0.25, color=color)
